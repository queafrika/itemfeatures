...
from itemfeatures.itemfeatures.override.monkey_patches import  get_valuation_rate, get_bin
...
class ExtStockEntry(StockEntry):
	...
	def set_basic_rate(self, reset_outgoing_rate=True, raise_error_if_no_rate=True):
		...
		scrap_item_qty = sum(d.qty for d in self.items if d.is_scrap_item)
	
		scrap_item_basic_rate = 0.0
		if scrap_item_qty:
			scrap_item_basic_rate = outgoing_items_cost / (finished_item_qty + scrap_item_qty)

		items = []
		for d in self.get("items"):
			if d.is_scrap_item:
				d.basic_rate = flt(scrap_item_basic_rate)
				d.basic_amount = flt(flt(d.qty) * flt(d.basic_rate), d.precision("basic_amount"))     
